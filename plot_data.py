...
# Number of sessions
d = np.diff(np.unique(total_sessions)).min()
left_of_first_bin = total_sessions.min() - float(d)/2
right_of_last_bin = total_sessions.max() + float(d)/2

ax0 = fig.add_subplot(gs[0, 0])
ax0.hist(total_sessions, np.arange(left_of_first_bin, right_of_last_bin + d, d), rwidth=1, density=True)
